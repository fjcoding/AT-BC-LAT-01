...
describe('Test for class Parser', () => {
    test ('The method have to create and appropiate array like the one in the schema and put the flag, value and type as attributes', () => {
        ...
        expect(argsIntance.createArgsSchema(['-p', '6023', '-l'], [0, 2])).toEqual([
    ...
    test ('The method have to tell us if the comman-line is valid or not using the array that has the flags and valueType', () => {
        const argsIntance = new Parser;
        expect(argsIntance.schemaValidation(['l boolean', 'p number', 'd string'], ['p number', 'd string'])).toEqual(true);
        expect(argsIntance.schemaValidation(['l boolean', 'p number', 'd string'], ['f number', 'd string'])).toEqual(false);
        expect(argsIntance.schemaValidation(['l boolean', 'p number', 'd string'], [])).toEqual(false);
    });
    test ('the method will print the flag and value', () => {
        const argsIntance = new Parser;
        expect(argsIntance.flagAndValue([
            { flag: 'l', value: true, valueType: 'boolean' },
            { flag: 'p', value: 8080, valueType: 'number' },
            { flag: 'd', value: '/usr/logs', valueType: 'string' }
        ])).toEqual([
    ...
    test ('The method will print the command-line input parsed depending on the validation', () => {
        const argsIntance = new Parser;
        expect(argsIntance.showArgsFlagAndValue(true, [
            'p 8596',
            'l true'
        ])).toEqual([
            'p 8596',
        ...
        expect(argsIntance.showArgsFlagAndValue(false, [
            's 8596',
            'l true' 
